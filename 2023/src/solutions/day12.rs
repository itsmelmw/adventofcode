...
type Cache = HashMap<(Vec<Status>, Vec<usize>, usize), usize>;
...
impl Solution for Day12 {
    ...
    fn solve_part_1(&self) -> String {
        ...
        self.rows
            .iter()
            .map(|row| self.solve_row(&row.springs, &row.contiguous, 0, &mut cache))
            .sum::<usize>()
            .to_string()
    }
    fn solve_part_2(&self) -> String {
        ...
        self.rows
            .iter()
            .map(|row| {
                let springs = vec![row.springs.clone(); 5].join(&Status::Unknown);
...
impl Day12 {
    fn solve_row(
        ...
        let cache_key = (springs.to_vec(), contiguous.to_vec(), cur_contiguous);
        ...
        let v = if let Some((curr, tail)) = springs.split_first() {
            ...
            (contiguous.is_empty() || (contiguous.len() == 1 && contiguous[0] == cur_contiguous)) as usize
        };
