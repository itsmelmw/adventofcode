...
fn solve12(blueprints: &[Blueprint], time_limit: usize) -> Vec<usize> {
    blueprints
        .iter()
        .map(|blueprint| {
            ...
            do_dfs(blueprint, &maximums, &State::start(), time_limit)
        })
        .collect::<Vec<usize>>()
}

fn do_dfs(blueprint: &Blueprint, maximums: &Materials, state: &State, time_limit: usize) -> usize {
    ...
    if state.robots.obsidian > 0 {
        let ore_wait = 1 + if state.inv.ore >= blueprint.geode_cost.ore {
            ...
            ((blueprint.geode_cost.ore - state.inv.ore) + state.robots.ore - 1) / state.robots.ore
        };
    ...
    if state.robots.clay < maximums.clay {
        let ore_wait = 1 + if state.inv.ore >= blueprint.clay_cost.ore {
            ...
            ((blueprint.clay_cost.ore - state.inv.ore) + state.robots.ore - 1) / state.robots.ore
        };
...
impl ProblemSolver for Solver {
    ...
    fn solve_part_2(&self) -> String {
        solve12(&self.blueprints[0..self.blueprints.len().min(3)], 32)
            .iter()
            .product::<usize>()
            .to_string()
    }
