    ...
    T: Add<Output = T> + Sub<Output = T> + Num + Copy + PartialEq,
{
    ...
    pub fn dir(&self, dir: &Dir) -> Self {
        match dir {
            Dir::Up => Point::new(self.x, self.y - T::one()),
            Dir::Right => Point::new(self.x + T::one(), self.y),
            Dir::Down => Point::new(self.x, self.y + T::one()),
            Dir::Left => Point::new(self.x - T::one(), self.y),
        }
    }
